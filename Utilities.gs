...
function getCachedSpreadsheet() {
  if (!_cachedSpreadsheet) {
    ...
    if (!key) {
      throw new Error('Spreadsheet key not found. Run setup() first.');
    }
...
/**
 * Normalize status values into canonical labels with new status system
 * @param {*} status - Raw status value
 * @returns {string} Normalized status in kebab-case format
 */
/**
 * Normalize status values into canonical labels with new status system
 * @param {*} status - Raw status value
...
function normalizeStatus(status) {
  ...
  // Direct mapping for new status format (kebab-case)
  const directMapping = {
    'new-order': 'new-order',
    'ordered': 'ordered',
  ...
  if (directMapping[s]) {
    return directMapping[s];
  }
  
  // Also check for space-separated versions (from Google Sheets)
  const spaceVersion = String(status).trim().toLowerCase().replace(/\s+/g, ' ');
  const displayToKebab = {
    'new order': 'new-order',
    'ordered': 'ordered',
    'delivered to mailroom': 'delivered-to-mailroom',
    'completed': 'completed',
  ...
  if (displayToKebab[spaceVersion]) {
    return displayToKebab[spaceVersion];
  }
  
  // Legacy status mapping for backward compatibility
  const statusGroups = {
    'new-order': ['pending', 'new', 'awaiting-approval', 'awaiting approval', 'awaiting-review', 'awaiting review', 'todo'],
    'ordered': ['processing', 'in-progress', 'in progress', 'order-placed', 'order placed', 'purchasing', 'buying'],
    'delivered-to-mailroom': ['delivered', 'received'],
    'completed': ['complete', 'done', 'fulfilled', 'picked-up', 'picked up'],
    'awaiting-club-response': ['on-hold', 'on hold', 'hold', 'paused', 'suspended', 'awaiting-response', 'awaiting response', 'waiting'],
    'cancelled': ['canceled', 'void', 'refunded']
  };
  
  for (const newStatus in statusGroups) {
    if (statusGroups[newStatus].includes(s) || statusGroups[newStatus].includes(spaceVersion)) {
      return newStatus;
    }
  }
